...
import static water.util.ArrayUtils.maxIndex;
    ...
    if (!regression) {
      ...
      for (int i = 0; i < _columnDistsRegression.length; ++i)
        if(!data.isIgnored(i)) {
          ...
          int colBins = c._isByte ? maxValue(c._rawB) : c._binned.length;
