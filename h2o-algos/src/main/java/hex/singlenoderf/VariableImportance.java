...
import static water.util.ArrayUtils.add;
 ...
 */
public class VariableImportance extends MRTask<VariableImportance> {
...
//  /* @OUT */ private float[/*ntrees*/] _sse;   // Sum of squared errors per tree (for regression only)
  ...
  private void init(Vec resp) {
    ...
    if (resp.domain()!=null) {
      assert respData.domain() != null;
      _model_classes_mapping = new int[resp.domain().length];
      _data_classes_mapping  = new int[respData.domain().length];
      // compute mapping
      alignEnumDomains(resp.domain(), respData.domain(), _model_classes_mapping, _data_classes_mapping);
    } else {
      assert respData.domain() == null;
...
//        int row = r + (int)chks[0]._start;
      ...
      ArrayUtils.shuffleArray(_oobs, oobcnt, soob, seedForOob, 0); // Shuffle array and copy results into <code>soob</code>
      for(int j = 0; j < oobcnt; j++) {
        int row = _oobs[j];
        // Do scoring:
        // - prepare a row data
        for (int i=0;i<chks.length - 1;i++) {
          data[i] = chks[i].atd(row); // 1+i - one free is expected by prediction
        }
        // - permute variable
        if (_var>=0) data[_var] = chks[_var].atd(soob[j]);
        else assert false;
        // - score data
        // - score only the tree
  ...
  @Override public void reduce( VariableImportance t ) {
    add(_votesOOB, t._votesOOB);
    add(_votesSOOB, t._votesSOOB);
    add(_nrows, t._nrows);
  }
