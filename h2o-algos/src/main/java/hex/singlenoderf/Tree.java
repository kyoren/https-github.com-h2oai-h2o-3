...
import static water.util.UnsafeUtils.get4;
...
public class Tree extends H2OCountedCompleter {
  ...
  boolean        _local_mode;
  boolean        _score_pojo;
  public hex.tree.CompressedTree compressedTree;
...
//    ex.printStackTrace();
  ...
  private StringBuffer computeStatistics() {
    ...
    for (int i = 0; i < _data.columns()-1; i++) {
      ...
      for (SplitInfo si : colSplitStats) {
        sb.append(", <=").append(PrettyPrint.p2d(si.splitNode().split_value())).append('{').append(si.affectedLeaves()).append("}x"+si._used+" ");
      }
  ...
  @Override public void compute2() {
    if(Job.isRunning(_jobKey)) {
      water.util.Timer timer  = new water.util.Timer();
...
//      System.out.println(matcher.group(1));
  ...
  static class SplitNode extends INode {
    ...
    @Override public StringBuilder toString( StringBuilder sb, int n ) {
      sb.append(_name).append("<=").append(PrettyPrint.p2d(split_value())).append('@').append(leaves()).append(" (");
    ...
    @Override public StringBuilder toJava( StringBuilder sb, int n, int... depth) {
      ...
      sb.append("Double.isNaN(data["+Integer.toString(_column)+"]) || (float) data["+Integer.toString(_column)+"] /* "+_name+"*/ ").append("<= ").append(PrettyPrint.p2d(split_value())).append("\n");
      if( sb.length() > n ) return sb;
  ...
  public Key toCompressedKey() {
    AutoBuffer bs = new AutoBuffer();
    CompressedTree compressedTree = compress();
    Key key = Key.make((byte)1,Key.DFJ_INTERNAL_USER, H2O.SELF);
    DKV.put(key, new Value(key, compressedTree));
    return key;
  }
  ...
  public static float classify( AutoBuffer ts, Chunk[] chks, int row, int modelDataMap[], short badData, boolean regression ) {
    ...
    while( (b = (byte) ts.get1()) != '[' ) { // While not a leaf indicator
      ...
      float fdat = (float)chks[col].atd(row);
  ...
  public static int dataId( byte[] bits) { return get4(bits, 0); }
  public static long seed ( byte[] bits) { return get8(bits, 4); }
...
//    Log.info(Sys.RANDF, _tree.toString(new StringBuilder(), Integer.MAX_VALUE).toString());
  ...
  public CompressedTree getCompressedTree() {
