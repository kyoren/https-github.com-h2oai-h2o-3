...
//    _server.setThreadPool(p);
  ...
  public void registerHandlers(HandlerWrapper s) {
    ...
    context.addServlet(H2oDatasetBinServlet.class,   "/3/DownloadDataset.bin");
    context.addServlet(H2oModelBinServlet.class,   "/3/Models.bin/*"); 
  ...
  public static class H2oDatasetBinServlet extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest request,
                         HttpServletResponse response) throws IOException, ServletException {
      String uri = getDecodedUri(request);
  ...
  public static class H2oModelBinServlet extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest request,
                         HttpServletResponse response) throws IOException, ServletException {
      //String uri = getDecodedUri(request);
       // try {
        //Pattern p = Pattern.compile(".*/Models\.bin/(?<modelid>.*?)(\\.java)?");
        /* Matcher m = p.matcher(uri);
        boolean b = m.matches();
        if (!b) {
          setResponseStatus(response, HttpServletResponse.SC_BAD_REQUEST);
        ...
        String model_id = m.group("modelid");
        boolean use_hex = false;
        String f_name = request.getParameter("frame_id");
        String hex_string = request.getParameter("hex_string");
        if (f_name == null) {
